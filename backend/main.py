...
async def lifespan(app: FastAPI):
    ...
    bot_in_web_enabled = os.getenv("RUN_TELEGRAM_BOT_IN_WEB", "1").lower() not in {
        "0",
        "false",
        "no",
    }
    ...
    # These `print()` calls make the startup behavior visible in Render logs
    # even if FastAPI/Uvicorn logging configuration is different.
    print(
        f"[telegram] configured={telegram_configured} bot_in_web_enabled={bot_in_web_enabled}"
    )

    should_run_bot = telegram_configured and bot_in_web_enabled
        ...
        try:
            ...
            def _bot_runner():
                try:
                    ...
                    # Don't crash the web service if bot fails, but do log it.
                    logger.exception("Telegram bot thread failed")
                    return

            print("[telegram] starting polling thread inside web service")
            t = threading.Thread(target=_bot_runner, daemon=True)
            t.start()
            app.state.bot_thread_started = True
            app.state.bot_thread = t
            logger.info("Telegram bot thread started inside web service")
        except Exception:
            logger.exception("Could not start Telegram bot thread")
    else:
        if not telegram_configured:
            print("[telegram] not starting: TELEGRAM_BOT_TOKEN missing/empty")
        elif not bot_in_web_enabled:
            print("[telegram] not starting: RUN_TELEGRAM_BOT_IN_WEB disabled")
    yield
...
def health_check():
    return {
        ...
        "bot_in_web_enabled": os.getenv("RUN_TELEGRAM_BOT_IN_WEB", "1").lower() not in {"0", "false", "no"},
